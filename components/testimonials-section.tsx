...
// Sample testimonials data - replace with actual data from your repository
const testimonials = [
  {
    id: 1,
    name: "Petra Somen",
        ...
        <ScrollReveal direction="up">
          <div className="text-center">
            ...
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto font-sans leading-relaxed">
              Join the world's most ambitious brands who trust Aurea Intelligence to transform their customer
              interactions.
            </p>
          </div>
        </ScrollReveal>
              ...
              <div className="glass-card rounded-3xl p-8 backdrop-blur-md bg-white/10 border border-white/20 shadow-xl hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 group">
                <div className="flex items-center mb-6">
                  <div className="relative w-16 h-16 rounded-full overflow-hidden mr-4 ring-2 ring-primary/20">
                    <Image
                      src={testimonial.image || "/placeholder.svg?height=64&width=64&query=professional headshot"}
        ...
        <div className="relative">
          <div className="flex animate-scroll">
            {duplicatedCompanies.map((company, index) => (
              ...
              >
                <div className="bg-white/80 backdrop-blur-sm rounded-lg shadow-sm border border-border/50 hover:shadow-md transition-shadow duration-300 w-32 h-20 flex items-center justify-center p-3">
                  <Image
                    src={company.logo || "/placeholder.svg"}
                    alt={`${company.name} logo`}
                    width={100}
                    height={50}
                    className="object-contain opacity-70 hover:opacity-100 transition-opacity duration-300 max-w-full max-h-full"
                  />
                </div>
              </div>
            ))}
